...
// @ts-ignore
const $ = (e: string): HTMLInputElement => document.querySelector(e);
...
$("#backwards-toggle").addEventListener("change", function () {
  setBackwards(this.checked);
  this.blur();
});

$("#flip-toggle").addEventListener("change", function () {
  setFlip(this.checked);
  this.blur();
});

$("#control-toggle").addEventListener("change", function () {
  pipeControl = this.checked;
  this.blur();
});

$("#mode-control").addEventListener("input", function () {
  mode = +this.value;
});

$("#speed-control").addEventListener("input", function () {
  $("#speedDisplay").innerHTML =
    "Speed: " + Math.abs((playerSpeed = +this.value * (backwards ? -1 : 1)));
});
